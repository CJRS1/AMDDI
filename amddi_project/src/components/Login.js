...
export default function Login({ setUser, setIsLoggedIn }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await axios.post('https://amddibackend-production-2880.up.railway.app/login', {
                email,
                password,
            });
            ...
            if (response.status === 200) {
                ...
                setIsLoggedIn(true);
                /* const responseUsuario = await axios.get(`https://amddibackend-production-2880.up.railway.app/usuario_por_email/${email} */
                try {
                    const responseUsuario = await axios.get(`https://amddibackend-production-2880.up.railway.app/usuario_por_email/${email}`);
